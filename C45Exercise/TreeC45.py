...
class TreeC45:
    ...
    def CreateTree(self,data,targetColumnIndex,hs):
        ...
        for attributeCounter in range(0,targetColumnIndex): 
            ...
            gainRatio = self.calcGainRatio(columnData,hs)
            attributeGainRatios.append(gainRatio)
            
        maxGain = max(attributeGainRatios)
        attributeIndex = attributeGainRatios.index(maxGain)

        root = TreeNode()
        root.title = data[0][attributeIndex] + ' - Gain ratio: ' + str(maxGain)
        root.children = []
    ...
    def calcGainRatio(self, data,hs):
        possibleValues = set()
        ...
        for possibleValue in possibleValues:
            ...
            totalSplitInfo+= (-1 * (count/(len(data)-1)) * math.log2((count/(len(data)-1))))

        attributeGain = hs-totalEntropyInProbability
        gainRatio = attributeGain/totalSplitInfo
